...
`scripts/verify_render.js` 已内置**跨平台 Chrome 自动检测**（Windows/macOS/Linux），自动搜索常见安装路径 + PATH 回退，无需手动改路径。如果自动检测不到，可设置 `CHROME_PATH` 环境变量指向浏览器可执行文件。
...
- **自包含 HTML**:PDF.js 库 / PDF base64 数据 / Worker 源码全部内联到单个 `index.html` 中，零外部文件依赖，纯离线可用。PDF 数据绕开 `file://` 下的 CORS，Worker 通过运行时 Blob URL 创建同源 worker。
- **CDP 真实渲染验证**:`verify_render.js` 用真实事件循环等 canvas 画出来再截图,而不是 `--virtual-time-budget`(后者驱动不了 worker,截图会骗人)。
- **跨平台 Chrome 检测**:`verify_render.js` 自动搜索 Windows/macOS/Linux 常见安装路径，无需手动配置。
