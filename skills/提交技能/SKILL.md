...
跟用户确认三件事：① 要提交哪个技能？② 它的源文件夹在哪？③ 技能叫什么名字（这会成为 `skills/<技能名>/` 的目录名）。
...
```bash
mkdir -p "$REPO/skills/<技能名>"
cp -r <源技能文件夹>/* "$REPO/skills/<技能名>/"
```

- 单文件技能 → 写成 `skills/<技能名>/SKILL.md`。
...
> ✅ 已提交，PR 链接在这。**接下来等管理员审核合并，你这边什么都不用做了。** 合并后你的技能就进共享库了。
...
1. 打开 https://github.com/DigitalBiologist/agentic-skills ，进 `skills/` 目录，点 **Add file → Create new file**；文件名填 `提交的技能名/SKILL.md`（斜杠会自动建子目录），把你的技能内容粘进去。
